...
- [ ] シミュレータ/実機で、ホーム画面から5画面(投稿詳細・プロフィール・フォロワー/フォロー一覧・フィード・ストーリーズ)すべてに遷移し、データ入力・プレビュー表示ができることを手動確認
  - 本開発環境(CLIサンドボックス)にはAndroid/iOSエミュレータが存在しないため未実施。ユーザー側の実機・シミュレータでの確認が必要。
- [ ] アプリを再起動し、入力したデータが保持されていることを確認
  - 上記と同じ理由で未実施。Hiveへの永続化ロジック自体はコードレビュー上は実装済み。

## 完了条件

- 本タスクリストの全項目が完了していること
- `.steering/20260815-initial-implementation/requirements.md` の「4. 受け入れ条件」をすべて満たしていること
- **残作業**: Phase 7の実機/シミュレータでの手動確認2項目(開発環境の制約により未実施)
